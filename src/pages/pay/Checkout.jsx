...
import {
    BsTrash3, BsChevronRight, BsQrCode, BsTicketDetailed, BsChevronLeft
} from 'react-icons/bs'; // THÊM ICON MŨI TÊN (BsChevronLeft)
...
const Checkout = () => {
    ...
    const [isBackHovered, setIsBackHovered] = useState(false); // State cho hiệu ứng nút Back
    ...
    return (
        <div style={{ backgroundColor: '#f8f8f8', minHeight: '100vh', fontFamily: "'Inter', sans-serif", paddingBottom: '80px' }}>
            ...
            <div className="container mt-4" style={{ maxWidth: '1100px' }}>
                ...
                {/* ================= NÚT QUAY LẠI (SUY NGHĨ THÊM) ================= */}
                <button 
                    className="btn btn-link text-decoration-none px-0 mb-3 d-flex align-items-center shadow-none"
                    onClick={() => navigate('/cart')} // ĐỔI LINK NÀY NẾU TRANG GIỎ HÀNG CỦA BẠN CÓ TÊN ROUTE KHÁC
                    style={{ 
                        color: isBackHovered ? PANDORA_PINK : '#6c757d', 
                        fontSize: '0.95rem', 
                        fontWeight: '500',
                ...
                >
                    <BsChevronLeft className="me-1" style={{ fontSize: '0.85rem' }} /> Suy nghĩ thêm
                </button>
                        ...
                        <div className="position-sticky" style={{ top: '20px' }}>
                            ...
                            <div className="card border-0 shadow-sm rounded-4 p-4 mb-4">
                                ...
                                {cartItems.map((item, index) => (
                                    <div key={item.id} className={`d-flex mb-4 ${index !== cartItems.length - 1 ? 'border-bottom pb-4' : ''}`}>
                                        <div className="border rounded-3 p-1 me-3 d-flex justify-content-center align-items-center bg-white" style={{ width: '70px', height: '70px', flexShrink: 0, overflow: 'hidden' }}>
                                        ...
                                        <div className="flex-grow-1 position-relative">
                                            ...
                                            <div className="d-inline-flex align-items-center bg-light text-dark px-2 py-1 rounded-2 mb-2" style={{ fontSize: '0.8rem', cursor: 'pointer' }}>
                                                {formatVariant(item.attributes)} <BsChevronRight className="ms-1" style={{ fontSize: '0.7rem' }} />
                                            </div>
                                            
                                            <div className="d-flex justify-content-between align-items-center mt-2">
                                                <span className="fw-bold fs-6 text-dark">{formatCurrency(item.price)}</span>
                                                <div className="d-flex flex-column align-items-end">
                                                    <div className="d-flex align-items-center border rounded-2" style={{ height: '32px' }}>
                                                        ...
                                                        <input type="text" className="form-control form-control-sm border-0 text-center fw-bold shadow-none p-0 bg-transparent" value={item.quantity} readOnly style={{ width: '30px' }} />
                                                        <button className="btn btn-sm btn-light border-0 shadow-none px-2 text-muted fw-bold bg-transparent" style={{ width: '30px' }}>+</button>
                                                    </div>
                                                    {item.stockWarning && <small className="text-danger mt-1 fw-bold" style={{ fontSize: '0.75rem' }}>{item.stockWarning}</small>}
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div className="card shadow-sm rounded-4 border-0 p-4">
                                ...
                                <div className="d-flex justify-content-between mb-2">
                                    ...
                                    <span className="text-dark fw-medium">{formatCurrency(totalAmount)}</span>
                                </div>
                                ...
                                <div className="d-flex justify-content-between align-items-center mt-3 pt-3 border-top">
                                    ...
                                    <span className="fs-5 fw-bold" style={{ color: PANDORA_PINK }}>{formatCurrency(totalAmount)}</span>
                                </div>
                                <button
